...
export const KINDS_BY_DISPLAY_KIND: Record<string, (ResourceKindInfo & { token: string }) | undefined> =
    Object.fromEntries(
...
// the app resolves routes the same way. It lives in the shared types package because
// the backend resolves the same routes when it builds a namespace's contained-resource
// list, and the two sides must not build routes differently.
//
