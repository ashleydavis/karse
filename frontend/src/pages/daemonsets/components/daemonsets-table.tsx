...
const columns: ColumnDef<DaemonSet>[] = [
    ...
    },
    {
        ...
        header: "Namespace",
        // The daemon set's namespace links to its own detail page. The row navigates to
        // the daemon set, so the link stops its click from bubbling up to the row.
        cell: (info) => (
            <span onClick={(e) => e.stopPropagation()}>
                <ResourceRef kind="Namespace" name={info.getValue<string>()} testId="daemonset-row-namespace-link" />
            </span>
        ),
    },
...
export function DaemonSetsTable() {
    ...
    const filterableColumns: FilterableColumn[] = [
        ...
        ...collectLabelColumns(data?.daemonSets ?? []),
    ];
        ...
        <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-row gap-2 items-center">
                ...
                <TableFilter
                    ...
                    testIdPrefix="daemonsets-filter"
                />
                <ColumnConfigButton configurable={configurable} config={config} onChange={setConfig} />
            </div>
            <ScrollableTableContainer testId="daemonsets-table">
                <Table size="small">
                    <TableHead>
