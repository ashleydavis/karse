...
export function useColumnConfig<T>(tableId: string, columns: ColumnDef<T>[], defaultHidden: string[] = []): UseColumnConfigResult {
    ...
    //
    // Both the order and the visibility map are rebuilt on every render, so TanStack re-derives
    // each row's cells every render. That costs work on a long list but is not a correctness
    // problem: the tables pass `autoResetPageIndex: false`, which stops a rebuilt row model
