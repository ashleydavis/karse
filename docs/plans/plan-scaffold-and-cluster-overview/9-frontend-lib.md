...
Create the non-UI frontend support layer: type mirror, axios wrapper, query client, and the kube-context provider. Covers plan section 10 (substeps 23-25). Exempt from unit testing per project policy (exercised by the manual e2e flow and `scripts/smoke.sh`).

## Files to create

1. **`frontend/src/lib/kubectl-types.ts`**: hand-mirror the backend's `Context`, `ContextsResponse`, `NodeStatus`, `Node`, `ClusterOverview` types.
...
3. **`frontend/src/lib/query-client.ts`**: export `queryClient = new QueryClient({ defaultOptions: { queries: { retry: false, refetchOnWindowFocus: false, staleTime: 5_000 } } })`. Single shared client for the whole app.

4. **`frontend/src/lib/kube-context.tsx`**:
