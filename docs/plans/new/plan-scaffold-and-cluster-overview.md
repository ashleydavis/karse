...
**Source layout convention**: every source file lives under `src/`. The frontend splits its `src/` into `pages/`, `components/`, and `lib/`. The backend has `src/lib/` for reusable server-side modules shared across routes and adapters. Backend tests live under `backend/src/tests/`, mirroring the source directory tree. Tests are not co-located with the modules they cover.
...
4. [x] The audit path `"./logs"` was cwd-relative and silently depended on the process cwd. *(Fixed: the default path is `"../logs"` (relative to the backend's cwd `backend/`), resolving to the repo root `logs/`, and is now overridable via the `KARSE_LOGS_DIR` env var. Documented in claude.md, audit-log.md, and architecture.md.)*
...
13. [x] Vite proxy hardcodes `http://127.0.0.1:5172` while `index.ts` reads an overridable `KARSE_PORT`; setting the env var breaks the proxy. *(Fixed: step 22's vite proxy target now reads `process.env.KARSE_PORT ?? "5172"`, matching `index.ts`.)*
...
26. [x] `scripts/smoke-tests.sh` assumes `jq`, `curl`, and `bash` are present with no prerequisite note. *(Fixed: step 31 adds a top-of-script `command -v` check for `jq`/`curl` with a clear failure message; step 5's e2e prerequisites list them too.)*
...
│   └───────────────────────────────────────────────────────────┘  │
└─────────────────────────────┬────────────────────────────────────┘
                              │ HTTP (axios)  Vite proxy /api → :5172
                              ▼
┌──────────────────────────────────────────────────────────────────┐
│ Backend  http://127.0.0.1:5172   (Bun + Express 5)               │
│                                                                  │
│   server.ts                                                      │
...
Process layout during development: two long-running processes run concurrently from the repo root via `bun start` (or `bun run dev` for hot reload): `bun --watch src/index.ts` in `backend/` on port 5172 and `vite` in `frontend/` on port 5173. Vite proxies `/api/*` to the backend.
...
│       ├── tsconfig.json
│       └── src/
│           └── index.ts                        Context, ContextsResponse, NodeStatus, Node, ClusterOverview
├── backend/
│   ├── package.json
│   ├── tsconfig.json
│   ├── jest.config.js
...
│       ├── kubectl/
│       │   ├── kubectl-adapter.ts              free async functions; imports types from karse-types
│       │   └── __mocks__/
│       │       └── kubectl-adapter.ts          Jest manual mock
│       ├── lib/                                (reusable server-side modules)
│       ├── routes/
│       │   ├── contexts-route.ts
│       │   └── cluster-route.ts
│       └── tests/
...
└── frontend/
    ...
    └── src/
        ...
        │   └── nodes-table.tsx
        └── lib/
            ├── api-client.ts                       axios wrapper; imports types from karse-types
...
2. Create `/home/ash/projects/karse/claude.md`. Concise project-level guidance covering:
    ...
    - **Testing discipline**: every **backend** non-React TypeScript module has tests under `backend/src/tests/` (the one exception is `index.ts`, pure bootstrap wiring covered by `scripts/smoke-tests.sh`). Tests run with `bun run test` (which invokes Jest). React UI code is not unit-tested. **The frontend is not unit-tested at all** per project policy: this includes the non-React `frontend/src/lib/*.ts` modules (`api-client.ts`, `query-client.ts`, `font-awesome.ts`), which are exercised only by the manual e2e flow and `scripts/smoke-tests.sh`. So "every non-React module is tested" applies to the **backend**, not the frontend. Tests **never** use `test.skip` or `describe.skip`. Tests **always** use `describe` and `test`, never `it`. Tests must not be fudged: each assertion checks a specific value, fixtures use realistic shapes, fakes are not asserted against themselves. Where mocking a module is required, prefer Jest's `__mocks__` directory adjacent to the module being mocked.
    - **Run**: `bun start` (or `bun run dev` for hot reload) from the repo root starts both processes concurrently via bun workspaces: backend on port 5172, frontend on port 5173. Vite proxies `/api` to the backend. Bun workspace scripts run with each package's directory as cwd. The `scripts/smoke-tests.sh` explicitly `cd backend` before launching. The audit log directory is set by `KARSE_LOGS_DIR` (default `"../logs"`, resolving to the repo root `logs/` with cwd `backend/`).
    ...
    - **Audit log**: every kubectl invocation is logged via `audit(LOGS_DIR, "kubectl", args)` from `audit-log.ts` before the spawn. `LOGS_DIR` is a module-level constant in `kubectl-adapter.ts` set to `process.env.KARSE_LOGS_DIR ?? "../logs"`. With the backend's cwd `backend/`, the default resolves to the repo root `logs/`. The private `kubectl(args)` helper in the adapter is the only path that calls `run("kubectl", ...)`. Do not bypass it.
3. Create `/home/ash/projects/karse/docs/architecture.md`: repeat the architecture diagram (the updated one including `audit-log.ts` and the private `kubectl(args)` audit-then-run path); explain each layer; document 127.0.0.1-only bind (local-only, never deployed, no CORS needed); document how kubectl failures surface (adapter throws a plain `Error` with kubectl's stderr as the message; the route returns HTTP 500 with that message as JSON). Include a short **"Local-only threat model / accepted risks"** subsection documenting: (a) kubectl stderr is returned verbatim to the client (issue 24, may reveal cluster/kubeconfig detail; accepted because the local user already owns the kubeconfig); (b) there is no Host-header / DNS-rebinding guard (issue 25, the single mutating route `POST /api/contexts/current` is protected only by being a non-simple cross-origin JSON POST that forces a CORS preflight; a Host-header allowlist is listed as a roadmap item). Then continue: document the **read-only kubectl invariant** (adapter exposes only five hard-coded read functions; private `kubectl(args)` helper is the only call site for `run("kubectl", ...)`; create/write/edit commands must never be added) and the **audit log integration** (every adapter call goes through `audit(LOGS_DIR, "kubectl", args)` before the spawn; `LOGS_DIR` is `process.env.KARSE_LOGS_DIR ?? "../logs"`, resolving to the repo root `logs/` with cwd `backend/`; details and retention live in `docs/audit-log.md`).
...
7. Create two docs in this step.
    ...
    - **Where**: `logs/<YYYY>/<MM>/<DD>/<HH>.log` at the repo root, derived from the server's **local time** (not UTC). One file per local-hour, created on first write. The base directory is controlled by the `KARSE_LOGS_DIR` env var (default `"../logs"`, relative to the backend's cwd). Bun workspace scripts run with each package's directory as cwd, so the backend runs with cwd `backend/` and the default `"../logs"` resolves to the repo root `logs/`. The `scripts/smoke-tests.sh` also explicitly `cd backend` before launching. Set `KARSE_LOGS_DIR` to an absolute path to override.
    ...
    - **How to read**: standard Unix text tools. `tail -f logs/2026/05/29/16.log` to follow the current hour; `grep -r 'use-context' logs` to find every context switch; `cat logs/2026/*/*/*.log` for everything this year.
...
8. Create `/home/ash/projects/karse/.gitignore`: `node_modules/`, `dist/`, `.DS_Store`, `*.log`, `.env*`, `backend/dist/`, `logs/`, `frontend/dist/`. Do **not** ignore `bun.lock`.
9. Create `/home/ash/projects/karse/mise.toml` with `[tools]\nbun = "latest"`. Create `/home/ash/projects/karse/package.json` (workspace root): `"name": "karse"`, `"private": true`, `"workspaces": ["backend", "frontend", "packages/*"]` (the glob picks up any future package added under `packages/` without editing the root config), scripts: `"dev"` = `bun run --filter '*' dev`, `"start"` = `bun run --filter '*' start`, `"compile"` = `bun run --filter '*' compile`, `"test"` = `bun run --filter 'karse-backend' test`, `"smoke"` = `bash scripts/smoke-tests.sh`, `"tests:all"` = `bun run compile && bun run test && bun run smoke`, plus short aliases `"d"` = `bun run dev`, `"s"` = `bun run start`, `"c"` = `bun run compile`, `"t"` = `bun run test`, `"ta"` = `bun run tests:all`, `"sm"` = `bun run smoke`. A single `bun install` from the repo root installs all workspace dependencies and writes one `bun.lock` at the root. Do not run `bun install` inside `backend/` or `frontend/` separately.
### 3. Backend package

10. Create `/home/ash/projects/karse/backend/package.json` (no separate `bun install`; the root `bun install` handles all workspaces):
    ...
    - After `bun install` at the repo root, `bun run test` from `backend/` runs Jest with zero tests collected; that is the baseline.
...
12. Create `/home/ash/projects/karse/backend/jest.config.js`. The package is `"type": "module"`, so this is a plain ESM config (`export default {...}`); no TS loader is needed (it is not `.ts`):
    ...
    - `moduleNameMapper: { "^karse-types$": "<rootDir>/../packages/karse-types/src/index.ts" }` — maps the workspace package to its TypeScript source so Jest resolves it directly without transforming files inside `node_modules`.
...
14. Create the `packages/karse-types` workspace package (the contract between frontend and backend):

    - **`packages/karse-types/package.json`**: `"name": "karse-types"`, `"type": "module"`, `"private": true`, `"exports": { ".": "./src/index.ts" }`. No runtime dependencies.

    - **`packages/karse-types/tsconfig.json`**: `target: "ESNext"`, `module: "ESNext"`, `moduleResolution: "bundler"`, `strict: true`, `skipLibCheck: true`, `rootDir: "src"`.

    - **`packages/karse-types/src/index.ts`** exporting:
    ...
    No tests: the package contains only type aliases (no runtime code). After this step `bun install` from the repo root picks up the new workspace; `bun run test` is unchanged (still green).
...
16. Create `/home/ash/projects/karse/backend/src/kubectl/kubectl-adapter.ts` as a module of free async functions (no factory, no interface, no injection). It imports `run` from `../command-runner`, `audit` from `../audit-log`, and the five contract types (`Context`, `ContextsResponse`, `NodeStatus`, `Node`, `ClusterOverview`) from `"karse-types"` (the shared workspace package; there is no `kubectl-types.ts` file in the backend). Exports:
    ...
    Before the private helper, declare a module-level constant:
...
19. Create `/home/ash/projects/karse/backend/src/index.ts`:
    ...
    - Read `KARSE_PORT` env var (`process.env.KARSE_PORT`), default `5172`.
    - Read `KARSE_LOGS_DIR` env var (`process.env.KARSE_LOGS_DIR`), default `"../logs"`, into a `logsDir` const.
    - `await pruneOldLogs(logsDir);` (top-level await; ESM only; with the backend's cwd `backend/`, the default resolves to the repo root `logs/`).
    - `const app = createServer();`
    - `app.listen(port, "127.0.0.1", () => console.log("Karse backend listening on http://127.0.0.1:" + port));`

    **No unit tests for this step.** Justification: `index.ts` is pure dependency wiring (read env, call factories, call `listen`). A unit test would mock all three factories and assert their wiring, which tests the test, not the code. Coverage is provided end-to-end by `scripts/smoke-tests.sh` in step 31, which boots this exact entrypoint and asserts every HTTP endpoint responds. This is the only exception to "every code step has tests" in the plan.

    After this step, `bun run test` still passes (no new tests added, none broken).

### 9. Frontend package

20. Create `/home/ash/projects/karse/frontend/package.json` (no separate `bun install`; the root `bun install` handles all workspaces).
    ...
    - dependencies also include `karse-types: "*"` (the shared workspace package).
...
22. Create Vite + Tailwind wiring and the HTML/CSS shell. Three small files in one step (none of them is interesting alone). **Tailwind 4 is configured CSS-first**: the `@tailwindcss/vite` plugin plus the `@import "tailwindcss";` line in `index.css` are the entire configuration; content sources are auto-detected, so there is **no `tailwind.config.ts`** (that `content`-array file is a Tailwind 3 pattern and is intentionally omitted).
    - `/home/ash/projects/karse/frontend/vite.config.ts`: plugins `react()`, `tailwindcss()`; `server.port = Number(process.env.KARSE_FRONTEND_PORT ?? "5173")`; `server.proxy = { "/api": "http://127.0.0.1:" + (process.env.KARSE_PORT ?? "5172") }` (see issue 13).
...
The frontend `lib/` is exempt from unit testing per project policy: it is an axios wrapper, a TanStack Query client, a Font Awesome side-effect import, and a small React Context provider, all of which are exercised by the manual e2e flow in `docs/e2e-testing.md` and `scripts/smoke-tests.sh`. Types come from the `karse-types` workspace package; there is no `kubectl-types.ts` in the frontend.

23. Create the axios wrapper:
...
31. Create the `/home/ash/projects/karse/scripts/` directory if it does not already exist, then create `/home/ash/projects/karse/scripts/smoke-tests.sh` (`#!/usr/bin/env bash`, `set -euo pipefail`):
    - **Prerequisite check first** (issue 26): the script depends on `jq` and `curl` (and runs under `bash`). At the top, verify both are present, e.g. `for tool in jq curl; do command -v "$tool" >/dev/null 2>&1 || { echo "smoke-tests.sh requires '$tool' on PATH" >&2; exit 1; }; done`, failing with a clear message rather than an opaque mid-script error. `docs/e2e-testing.md` lists the same prerequisites (step 5).
    - Trap `EXIT` to kill the backend if still running.
    - `(cd backend && bun src/index.ts) &`; capture PID. (The backend runs with cwd `backend/`, so the default `KARSE_LOGS_DIR` value `"../logs"` resolves to the repo root `logs/`.)
...
32. Re-read every documentation file written in section 1 and reconcile it against what was actually built:
    ...
    - `docs/api.md`: every endpoint, method, request body, response body, status code, and error shape match `routes/contexts-route.ts` and `routes/cluster-route.ts`. Type definitions copied into the doc match `packages/karse-types/src/index.ts`.
    - `docs/e2e-testing.md`: UI behaviour described matches what the components actually render and what `scripts/smoke-tests.sh` actually asserts.
...
Captured as `/home/ash/projects/karse/scripts/smoke-tests.sh` (step 31). The AI agent runs `bash scripts/smoke-tests.sh` as part of Verify. It boots the real backend, exercises every HTTP endpoint, asserts JSON shape with `jq`, switches context if any exist, and builds the frontend.
...
2. `bun run ta` (`tests:all`): type-checks both packages, runs all unit tests, runs smoke tests, and builds the frontend. All must pass.
3. Confirm `git log` exits non-zero (the plan must not have created any commits).

## Notes

- **Every code step delivers tests in the same step**, except `index.ts` (step 19), which is bootstrap-only and covered by `scripts/smoke-tests.sh`. This exception is justified inline in step 19 and documented in `claude.md`.
...
- **Backend layout**: `src/kubectl/` for the adapter (types come from `karse-types`), `src/routes/` for route modules, `src/lib/` for reusable server-side code shared across routes and adapters, `src/tests/` for unit tests (mirroring the source tree).
...
- **Bun workspaces**: the root `package.json` declares `workspaces: ["backend", "frontend", "packages/*"]`. A single `bun install` at the repo root installs all workspace dependencies and writes one `bun.lock`. Running `bun start` or `bun run dev` at the root executes each workspace's script in parallel via `bun run --filter '*' <script>`. Each workspace script runs with its own package directory as cwd. The audit log directory is controlled by `KARSE_LOGS_DIR` (default `"../logs"`); with cwd `backend/`, the default resolves to the repo root `logs/`.
- **Why a shared types package**: `packages/karse-types` is the single source of truth for the five contract types shared between frontend and backend. Both declare `"karse-types": "*"` as a workspace dependency; bun resolves it via the `packages/*` glob. The backend's `jest.config.js` maps the package name to its TypeScript source via `moduleNameMapper` so Jest resolves it without transforming `node_modules`. Adding a new package under `packages/` requires no root config change.
