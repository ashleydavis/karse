...
The dropdown placement and keyboard shortcut for the context picker are covered under [quick-find](../quick-find/detail.md), and the grouping of the contexts page and the header dropdown by environment is covered under [cluster-environments](../cluster-environments/detail.md). Tear each cluster down with the Teardown step at the end of this doc.
...
- **Header context dropdown** (click it or press Ctrl+K): "No contexts match" message.
...
- **Header context dropdown** (click it or press Ctrl+K): both contexts listed and searchable.
...
- **Header context dropdown** (click it or press Ctrl+K): all five contexts listed. Type a partial name to filter and confirm only matching rows appear.
- **Switching**: use the header dropdown or Contexts page to select each context in turn. Confirm the overview tiles and nodes table update to show that cluster's data.
