...
- Look at the far right of the toolbar row that holds the Stream button. Two icon-only buttons sit there, clear of Stream and to its right: an up-chevron (jump to top) and a down-chevron (jump to bottom), with the down-chevron rightmost. They carry no text labels; hover each to see its "Jump to top" / "Jump to bottom" tooltip.
- Stream a pod (as above) and let the viewer fill past one screen so there is history to jump through.
- Press the up-chevron (jump to top). The view jumps straight to the very first streamed line. New lines keep arriving, but the view stays at the top: it is not yanked back down.
- Press the down-chevron (jump to bottom). The view jumps to the newest line, and it **sticks**: as further lines arrive the view stays pinned to the end, exactly as if you had scrolled back down by hand. It must not scroll to the end once and then be left behind by the next line.
- Repeat the top/bottom cycle a few times, and mix in the wheel and the scrollbar thumb. Every press of the down-chevron (jump to bottom) must re-engage the follow behaviour for the rest of the stream.
