...
- **CPU / Memory columns and sort**: the table has **CPU** and **Memory** columns showing each pod's consumption (sourced from the cluster Performance snapshot). On a cluster with a metrics-server these show real figures; on a metrics-less cluster (e.g. plain kwok) they show an em-dash (`—`). Click the **CPU** header to sort pods by CPU (highest first), click again to reverse to ascending; do the same with the **Memory** header and confirm the order reflects memory, not CPU. Pods with no usage reading sort to the bottom of the ascending order. (There are deliberately no disk or network columns: the Kubernetes Metrics API reports neither, so there is no consumption figure to show.)
