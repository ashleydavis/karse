...
- **CPU / Memory bar columns, toggles, and sort**: the table has **CPU** and **Memory** columns rendering an inline utilisation bar whose **percentage base is the pod's own request** (pod usage ÷ pod request, e.g. `80%`; sourced from the cluster Performance snapshot — distinct from the node-allocatable base used on the nodes table). Two toggles in the toolbar drive both columns together: **Usage | Requests** (view mode) and **% | Absolute** (value format). In **Usage** mode the bar fills to usage ÷ request; toggle to **Requests** and the bar shows the request itself as a full bar; toggle **Absolute** and the value switches to `used / request` (usage mode, e.g. `120m / 100m vCPU`) or the request quantity (requests mode, e.g. `250m`, `256Mi`). On a cluster with a metrics-server the usage-mode bars show real percentages; on a metrics-less cluster (e.g. plain kwok) usage shows an em-dash (`—`) and an empty bar while requests mode still populates from pod specs. Run with `KARSE_FAKE_METRICS=1` to see usage bars on kwok. Click the **CPU** header to sort by the shown percentage (highest first), click again to reverse; do the same with **Memory** and confirm the order reflects memory, not CPU. Pods with no usage reading (or no request set) sort to the bottom of the ascending order. (There are deliberately no disk or network columns: the Kubernetes Metrics API reports neither, so there is no consumption figure to show.)
- **Utilization status badge**: in usage mode a **Utilization** column shows a status badge grading each pod's usage ÷ request ratio — **Over-reserving** (≤ 35%), **Under-provisioned** (≥ 90%), or **OK** in between; an em-dash when the ratio cannot be computed. Toggle to **Requests** and the badge is empty (no ratio to grade).
...
### What to check
- **Pods page**: five rows in `default`, one per phase. The shared **Filter** button reads `Filter: All` (the status group is labelled **Status**, not "Phase").
...
### What to check
- **Pods page**: five rows. The shared **Filter** button reads `Filter: All`; the stats header reads `Healthy: 2` and `Error: 2`.
