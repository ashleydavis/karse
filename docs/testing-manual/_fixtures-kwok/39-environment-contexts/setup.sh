...
# is what lets this fixture use exactly the names the inference rules are about, instead of
# the kwokctl-generated `kwok-karse-test-*` names (every one of which carries a `test`
# segment, so none of them can ever demonstrate the Unassigned or Local groups).
#
