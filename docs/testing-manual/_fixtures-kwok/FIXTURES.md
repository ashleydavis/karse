...
| [36-real-shaped-labels](36-real-shaped-labels/) | The same pods twice, in a `shortlabels` and a `reallabels` namespace, differing only in label shape (two short labels vs the real-cluster Kubernetes/Helm label set), plus real-shaped deployments, stateful sets and daemon sets | resource-search |
