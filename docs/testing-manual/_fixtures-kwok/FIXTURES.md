...
| [36-real-shaped-labels](36-real-shaped-labels/) | The same pods twice, in a `shortlabels` and a `reallabels` namespace, differing only in label shape (two short labels vs the real-cluster Kubernetes/Helm label set), plus real-shaped deployments, stateful sets, daemon sets and autoscalers | resource-search |
| [37-large-pod-list](37-large-pod-list/) | One namespace (`bigpods`) holding a lot of real-cluster-shaped pods; the pod count is `setup.sh`'s first argument (default 1500) | resource-search |
| [38-generic-detail](38-generic-detail/) | Kinds with no detail page of their own: a namespaced autoscaler, service, job and lease in `default`, plus a cluster-scoped persistent volume | generic-detail |
| [39-environment-contexts](39-environment-contexts/) | No cluster: two handcrafted kubeconfigs in the git-ignored `fixtures-tmp/`, one covering every environment group (including `devops-prod`, which must read as Production) and one whose names match no token at all | cluster-environments |
| [40-overview-count-links](40-overview-count-links/) | Four nodes (one per CPU-requests band plus one reporting MemoryPressure) and five pods (one Pending, one previously OOMKilled, three carrying the requests that band their node) | cluster-overview, nodes-view, pods-view, resource-utilization |
