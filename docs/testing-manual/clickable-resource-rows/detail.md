...
- **All resources** (`/all-resources`): each namespaced row's **Namespace** cell is a link. Click it and confirm it opens that namespace. Watch the table for a few seconds first: it must sit completely still. (The table rebuilds the rows and columns it hands the table library on every render. That used to loop and re-mount every row continuously — the link was there, but it was destroyed under the pointer before a click could land, so it could not be clicked at all. The table now passes `autoResetPageIndex: false`, which is what breaks the loop.) A cluster-scoped row (Kind `Node` or `Namespace`) has an empty Namespace cell with no link.
- Kinds with no page of their own: an object such as a ReplicaSet links to the generic detail page (`/resources/replicasets/...`) rather than showing as plain text — on the errors/events tables and on the error/event detail pages alike. Only a reference Karse cannot resolve at all (an empty name, a namespaced kind with no namespace, a kind Karse will not read) stays plain text with no navigation on click.
