...
| [cluster-environments](./cluster-environments/index.md) | Group contexts by environment: name inference, hand-set labels, persistence, grouped pickers. |
...
| [multi-cluster-overview](./multi-cluster-overview/index.md) | All clusters page: totals across every context, per-cluster table, error rows, empty state. |
...
| [copy-button](./copy-button/index.md) | Shared copy-to-clipboard controls beside pasteable values: the plain button, the two-entry copy menu (short name, full path), the caret that marks a menu control, the rollout onto every resource the app names (detail page titles, every resource table row, and every referenced resource), and the Commands / YAML tabs. |
