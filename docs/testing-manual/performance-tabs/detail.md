...
- **Long node names are middle-truncated** (resource-utilization-5): a node whose name is too long for its box shows a `...`-truncated label (the start and end kept, the middle replaced with `...`) before the share percent, e.g. `node-wi...rker-01 30%`, and the tooltip title shows the same truncation. To observe this, add a long-named node to the `FAKE_METRICS.nodes` list while testing. The truncation is display-only — clicking still navigates to the full node name.
