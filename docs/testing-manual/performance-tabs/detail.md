...
- **Long node names are middle-truncated in the box label** (resource-utilization-5): a node whose name is too long for its box shows a `...`-truncated label (the start and end kept, the middle replaced with `...`) before the share percent, e.g. `node-wi...rker-01 30%`. The hover **tooltip title shows the full, untruncated node name** on a single line — the tooltip widens to fit it rather than wrapping the name onto many lines in a narrow box. To observe this, add a long-named node to the `FAKE_METRICS.nodes` list while testing. The truncation is display-only — clicking still navigates to the full node name.
