...
| [timestamp-format](./timestamp-format/index.md) | Settled | Complete | Header toggle switching every timestamp in the app between its age and the absolute local time, persisted. |
...
| [copy-button](./copy-button/index.md) | Settled | Partial | Shared copy-to-clipboard button beside pasteable values, plus the single clipboard helper. Rolled out on the pod detail page so far. |
