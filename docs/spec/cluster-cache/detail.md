...
- Refresh re-fetches **every** view, on whatever page it is clicked: it reloads all client-side queries rather than a named subset, so each page currently on screen (Cluster, Nodes, Pods, Deployments, StatefulSets, DaemonSets, Events, Errors, All resources, the detail pages, and the performance panels) issues its own fresh request. This is a requirement, not an implementation detail: a page added later must be refreshed without anyone remembering to register its query key. The page queries are therefore selected by *excluding* the app-shell keys, never by naming the page keys.
- Refresh **empties the current page's content and reloads it behind the page's own spinner**. Clicking it returns the page's queries to their pending, dataless state, so the list, detail or panel it renders clears and its usual loading indicator shows until the fresh data lands. This is a requirement: a refresh must read as a genuine reload of the content. Refetching in the background instead would retain the query's data, leaving the page rendered untouched and swapping the rows in silently — and when the data is unchanged nothing on the page would change at all, which is what made refresh look dead. It applies to every page that shows cluster data, lists and detail pages alike.
- The `["contexts"]` query is app shell, not page content, so it is re-fetched **without** being emptied. Every page query embeds the selected context in its key and is gated on it, so blanking the context list would disable those queries rather than put them in flight (an empty page, not a reloading one), and would re-disable the refresh button for the length of the contexts re-fetch.
- The refresh button gives visible feedback so a click is unmistakably acknowledged even when the refetched data is identical: briefly on click the icon spins and the button is disabled (in-progress) and a bottom "Refreshing…" toast shows, and on completion it briefly shows a check and a "Refreshed" toast before returning to its resting state. The prominent toast is what makes the acknowledgement unmissable; the small header icon alone is easy to overlook. This navbar acknowledgement is separate from, and independent of, the page emptying and reloading behind its spinner.
- This feedback is driven on a timer, not on the page reload completing. Reloading all queries returns a promise that only settles once every active refetch has (the shared cluster-performance query among them), so gating the feedback on it held the acknowledgement hostage to the slowest request on the page: on a cluster with no Metrics API the performance refetch does not come back promptly and aborts only at the 15s load timeout (`LOAD_TIMEOUT_MS`), which pinned the button in the in-progress state for that whole window and delayed the acknowledgement and re-enable by some 15 seconds — long enough to read as a dead button (reported on the Cluster page). The refetch requests still fire; the acknowledgement simply does not wait on them.
...
- [x] Refresh empties the current page's content and shows that page's own loading spinner while it reloads, on lists and detail pages alike, so it reads as a genuine reload rather than a silent background swap.
