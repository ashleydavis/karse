...
- This feedback is driven on a timer, not on the query invalidation completing. Invalidating all queries returns a promise that only settles once every active background refetch has (the shared cluster-performance query among them), and such a refetch can stay pending indefinitely; gating the feedback on it pinned the button in the in-progress state forever on whatever page hit a non-settling refetch (seen on the Cluster page). The refetch requests still fire; the acknowledgement simply does not wait on them.
