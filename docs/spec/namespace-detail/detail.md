...
- The Status tab shows the namespace's phase, age, labels, annotations, resource quotas (each quota's hard limits), and limit ranges (one row per limit type/resource with min/max/default).
