...
- The Status tab shows the namespace's status (its lifecycle phase, e.g. Active/Terminating, under a row labelled "Status"; phase and status are the same concept here, see the **Status vs phase naming** note in `resource-search`), age, a **Resources** count, labels, annotations, resource quotas (each quota's hard limits), and limit ranges (one row per limit type/resource with min/max/default).
