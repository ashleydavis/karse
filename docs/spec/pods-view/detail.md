...
- **CPU and Memory columns** show each pod's resource consumption **as a percentage of the node it runs on** (pod usage ÷ that node's allocatable, e.g. `12%`) so the user can sort to find the heaviest consumers and the figures are comparable across pods on differently-sized nodes. Per-pod usage and per-node allocatable both come from the cluster Performance snapshot (`GET /api/cluster/performance` → `{ nodes: NodeUsage[], pods: PodUsage[] }`); each pod's usage is joined by namespace/name and divided by the allocatable of its scheduling node (`pod.node`, the same node shown in the Node column). Both columns sort by that percentage; a pod with no usage reading, or whose node/allocatable is unknown (or when the Metrics API is unavailable), shows an em-dash and sorts below every pod that has a reading. Only CPU and memory are shown: the Kubernetes Metrics API reports neither disk nor network consumption (the documented exclusion in [performance-tabs](../performance-tabs/detail.md)), so there is no disk/network figure to show or sort by.
...
- [x] The table has CPU and Memory consumption columns, each showing the pod's usage as a percentage of its node's allocatable and each sortable ascending/descending by that percentage. (Disk and network are excluded: the Metrics API reports neither — see [performance-tabs](../performance-tabs/detail.md).)
