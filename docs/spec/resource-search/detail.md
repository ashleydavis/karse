...
- **Typing stays responsive however long the list is.** The characters appear in the search box the instant they are typed, and the table keeps up: a query typed at speed into a table of a few thousand rows does not freeze the UI. Filtering is deferred rather than run once per character (`use-search-filter.ts`: the box is bound to the typed text, the table's global filter to a deferred copy of it), so the rows re-filter as typing pauses rather than on every keystroke, and the rows that survive a query are memoised (`data-table-row.tsx`) so they are not re-created and re-styled when the query changes. The rows a query selects are exactly the rows it selected before: only the timing of the re-render changes, never the result.
...
- [x] Typing into a search box never blocks the UI: the typed text appears immediately, and the table re-filters without a whole-table re-render per character, so a table of a few thousand rows still types smoothly.
