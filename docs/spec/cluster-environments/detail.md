...
- Each row of the contexts page carries an **Environment** column: the resolved environment as a chip, plus a selector offering *Auto (from name)* and every environment in the user's list. Unassigned is not offerable, because it is not in the list; *Auto* already hands the decision back to the name.
