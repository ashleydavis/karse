...
- The Logs page's toolbar carries **icon-only jump-to-top and jump-to-bottom buttons** (an up-chevron and a down-chevron, no text labels; each carries a tooltip and `aria-label` of "Jump to top" / "Jump to bottom"), pushed to the far right of the row so they sit clear of the Stream/Stop button. Jump-to-top scrolls the viewer to its first line; jump-to-bottom scrolls it to its last line **and re-engages auto-follow**, so the view then stays locked to the end as new lines arrive, exactly as if the user had scrolled back to the bottom by hand (not a one-off scroll that the next line leaves behind). Both jumps derive the follow flag from the position they land on, the same rule the wheel, keyboard and thumb-drag paths use, so jump-to-top stops following and jump-to-bottom resumes it. The buttons are part of the Logs page's picker toolbar, so they do not appear on the Pod detail Logs tab (which has no toolbar).
...
- [x] The Logs page has icon-only jump-to-top and jump-to-bottom buttons (up- and down-chevron icons, no text labels) at the far right of the toolbar row (right of the Stream/Stop button): jump-to-top scrolls the viewer to its first line and stops following; jump-to-bottom scrolls it to its last line and re-engages auto-follow, so the view then stays pinned to the end as new lines arrive.
