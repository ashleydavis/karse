...
- The Name column carries the two-form copy menu beside each stateful set name, and so does the Namespace column beside it. See [copy-button](../copy-button/detail.md).
...
- A stats header above the table shows Total / Healthy / Error chips for the current scope; Healthy counts stateful sets with all desired replicas ready (`x/x`, `x > 0`), Error counts those with none ready (`0/x`, `x > 0`) (see `resource-stats`).
