...
- The context picker's trigger is labelled with the active context's name (`no context` when none is active) and carries the environment chip beside it. Its dropdown lists contexts filtered by name or cluster, sorted by name, marking the active one with a chip; selecting a row switches the tab's active context. Its rows are listed under one subheading per environment, applied after the filter, so a search that hides every context in an environment hides that subheading too. Each subheading names its environment with the shared environment chip, the same badge the top bar shows beside the dropdown, so one environment looks the same in both places (see [cluster-environments](../cluster-environments/detail.md)).
...
- [x] The dropdown's environment subheadings are drawn with the shared environment chip, not plain text, so they match the environment badge in the top bar.
