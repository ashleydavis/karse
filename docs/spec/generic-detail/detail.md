...
Backed by: `packages/karse-types/src/index.ts` (`RESOURCE_KINDS` and `knownResourceKind`, the shared table of kinds Karse knows; `isReadableResourceKind`, the check the backend applies; `ResourceKindInfo` and `ResourceDetail`), `backend/src/kubectl/kubectl-adapter.ts` (`getResourceDetail`, `getResourceYaml`), `backend/src/routes/resource-route.ts` (`GET /resource/:type/:name`), `frontend/src/pages/resource-detail/index.tsx` (the page), `packages/karse-types/src/index.ts` (`resourcePath` and `GENERIC_DETAIL_ROOT`, shared with the backend), `frontend/src/lib/resource-link.ts` (`resourceNameSegments`, `resourceKindLabel`), `frontend/src/app.tsx` (the two routes) and `frontend/src/lib/breadcrumb-trail.ts` (the origin trail).
