...
- The workload name in the heading and the Namespace field each carry the two-form copy menu, as do the Name and Node columns of the Pods tab's table. See [copy-button](../copy-button/detail.md).
...
- The page renders metadata, the stat counters, and events on the Status tab, and exposes the workload's pods on a dedicated **Pods** sub-tab (each row links to its pod detail page; a clear empty state shows when the workload owns no pods). The Pods sub-tab shows a resource-stats header (Total / Healthy / Error) above the pod table, reusing the shared `ResourceStatsHeader` component and the Pods stat helper (see `resource-stats`). The counts are computed client-side from the same owner-scoped `pods[]` list the tab already holds, so they update on refetch / context / namespace change, render zeroed in the empty state (Total 0 / Healthy 0 / Error 0), and add no extra backend call. The Status tab still shows the workload's selector inline; its own labels live on a dedicated **Labels** sub-tab as a searchable, sortable Key / Value table (see `labels-tab`). It offers guided commands for the workload (see `guided-commands`) and a raw-YAML view (see `yaml-viewer`).
