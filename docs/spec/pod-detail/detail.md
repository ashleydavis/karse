...
- The page's pasteable values each carry a copy control (see `copy-button`). The pod name in the heading and the Namespace and Node fields on the Status tab are resource names, so they carry the two-form copy menu (short name, full path); the Pod IP, and each container's image in the Containers and Init Containers tables, carry the plain single-click button. Each container row's Name cell carries the copy menu, whose full path extends the pod's. Age has none (a rendered duration, not an identifier), and a field showing the `-` placeholder has none either.
...
- [x] The pod name, namespace, node, Pod IP, each container name and each container image have a copy control (the two-form menu on the names, the plain button on the rest); Age and placeholder values do not.
