...
A sub tab on each resource detail page that shows the raw YAML of that resource. The kind is checked before it is used, so a caller cannot turn this into a way of running kubectl with arbitrary arguments or of reading a kind Karse refuses to show.

Backed by: `GET /api/yaml/:type/:name`, `backend/src/routes/yaml-route.ts`, `backend/src/kubectl/kubectl-adapter.ts` (`getResourceYaml`), `packages/karse-types/src/index.ts` (`isReadableResourceKind`, `RESOURCE_KINDS`), `frontend/src/components/yaml-tab-panel.tsx`.
...
- `type` is the kind's kubectl resource name, and is accepted for any kind the cluster serves, matching the generic detail page it backs (see [generic-detail](../generic-detail/detail.md)): the six kinds with their own detail pages (`nodes`, `pods`, ...), the common kinds Karse knows (`replicasets`, `jobs`, `horizontalpodautoscalers`, ...) and a kind it does not know at all (a custom resource, a `leases`). It is rejected (`Karse will not read resources of type: <type>`) only when it does not look like a kubectl resource name, or when it names a kind Karse refuses to read. Secrets are refused.
