...
- **Resource count.** `resourceCount` is the number of **pods** in the namespace. "Resources" is defined as pods: pods are the unit of running workload, present in every namespace, and already the count surfaced on the cluster overview, so the two views stay consistent. The same pods-only definition is used by the namespace detail page's Details-tab Resources stat (see `namespace-detail`), so the same namespace shows the same number on the list and the detail page. The counts come from a single cluster-wide `kubectl get pods -A` call (not one call per namespace), so the cost does not grow with namespace count. The pod query runs in parallel with the namespace list and is tolerated: if it fails, every namespace is still returned with `resourceCount: null` and the table renders, showing an em-dash in that column. The namespaces page shows a sortable **Resources** column with the per-namespace count.
