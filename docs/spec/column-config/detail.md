...
- The destination shows a drop-target indicator: a gap opens at the insertion point marking where the column will land on release. For a reorder within a section, dnd-kit's sortable strategy shifts the other rows to open the gap. For a move between sections, the dragged column is placed live into the target section at the cursor's position, so the gap appears in the destination at the insertion point.
- On release, the column lands exactly where the drop-target indicator showed, for both within-section reorders and moves between sections. A cross-section drop into the middle of the destination list lands at that middle position, not appended to the end. Dropping a cross-section column over the destination's bare area below its last row lands it at the end of that list, so every slot (start, middle, end) is reachable.
