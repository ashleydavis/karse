...
- **Readable resource kinds** (`packages/karse-types`, `RESOURCE_KINDS`): one table of every kind Karse will read, mapping the URL/API token to that kind's display kind, the resource name handed to `kubectl get`, and whether it is namespaced. Both sides share it deliberately: the backend uses it as the read whitelist (so a caller-supplied kind can never reach the kubectl argument list), and the frontend uses it to decide which kinds a resource reference links to and which carry a namespace segment. That is why the generic detail page and the raw-YAML endpoint can never disagree with each other about a kind. See `docs/spec/generic-detail`.
