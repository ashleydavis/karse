...
- **Context environments**: the contexts page, the header dropdown and the `Ctrl+K` quick-picker group contexts by environment. The whole rule (the name tokens, the label override, the fixed group order) lives in one non-React module, `lib/cluster-environments.ts`, which all three import, so there is no second copy to drift; the labels that override it are a field of the `karse-config` entry above. It is purely a display concern: no backend route, no kubectl call, nothing written to the kubeconfig. See `docs/spec/cluster-environments`.
